package com.example.jeppe.weatherapp;

/**
 * Created by Søren on 06-11-2017.
 */

public class Globals {
    public static final String WEATHER_SERVICE_ADD_CITY_WEATHER = "add_city_city_weater";
    public static final String WEATHER_SERVICE_ADD_CITY_WEATHER_DATA = "add_city_city_weater_data";
    public static final String CITY_DETAILS_SINGLE_CITY = "city_details_single_city";
    public static final String CITY_DETAILS_BUNDLE = "city_details_bundle";
    public static final int REMOVE_CITY = 201;
}
